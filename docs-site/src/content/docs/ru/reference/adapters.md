...
**Назначение:** локально установленный Devin CLI, управляемый по Agent Client Protocol (`devin acp`, построчный JSON-RPC поверх stdio).
**Аутентификация:** opencodex ничего не хранит. Учётные данные после `devin auth login` держит сам CLI.

- Только `runTurn`: у рукопожатия через дочерний процесс нет запроса для общего транспортного пути.
- Один ход — одна сессия ACP: `initialize`, `session/new`, `session/prompt`, между ними приходят уведомления `session/update`.
- Собственные вызовы инструментов CLI наружу не уходят. Devin выполняет их внутри сессии, и передача их как клиентских инструментов либо срывает ход, либо приводит к двойному выполнению.
- **Запросы разрешений по умолчанию отклоняются.** Этот провайдер запускает агента в рабочем дереве пользователя, поэтому разрешение требует `OPENCODEX_DEVIN_CLI_ALLOW_TOOLS=1`, а дочерний процесс получает урезанное окружение, а не окружение прокси.
- Исполняемый файл ищется в `OPENCODEX_DEVIN_CLI_BIN`, затем по официальным путям установки, затем в `PATH`.
