...
| `devin` | `devin` | `https://server.codeium.com` | **チャットは未検証です。実際の無料アカウントではログインとモデル取得は成功しますが、`GetChatMessage` が毎回不透明な `invalid_argument` を返しターンが完了しません。有料アカウントは未確認です。ターンが完了する Devin 経路が必要なら `devin-cli` を使ってください。** 実験的な非公式 Cognition/Devin ブリッジ。ログインは Auth0 のブラウザサインインを開き、取得したトークンを `RegisterUser` で長期 API キーに交換します。モデル一覧は `GetCascadeModelConfigs` でアカウントごとに取得し、ストリーミングは Connect-RPC 上の `runTurn` 経路のみを使います。ダッシュボードのプリセットには既定で含まれません。 |
| `devin-cli` | `devin-cli` | `devin://acp/stdio` | ローカルにインストールされた Devin CLI を Agent Client Protocol（`devin acp`、stdio 上の JSON-RPC）で駆動します。CLI が `devin auth login` の資格情報を保持するため、opencodex 側はキーを保存しません。実行ファイルは `OPENCODEX_DEVIN_CLI_BIN` で指定でき、CLI にファイル操作を許可するには `OPENCODEX_DEVIN_CLI_ALLOW_TOOLS=1` の明示が必要です（既定は拒否）。 |
